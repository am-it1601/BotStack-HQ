...
- [ ] [backend] Add `class-validator` + `class-transformer` deps and wire `@nestjs/common` `ValidationPipe`, then decorate the Workspace DTOs (UpdateWorkspaceDto/InviteMemberDto/UpdateMemberDto) with validation rules documented in their JSDoc (agentName non-empty ≤50, valid email, enum membership) — deferred: new deps require flagging; boilerplate-only task left DTOs as plain classes — (raised: 2026-06-09, by: workspace-team-apis-boilerplate)
- [ ] [backend] Add `@nestjs/swagger` dep and complete Swagger/OpenAPI annotations on all 5 Workspace & Team endpoints (DOD requirement) — deferred: new dep requires flagging; boilerplate-only task documented endpoints via JSDoc TODOs instead — (raised: 2026-06-09, by: workspace-team-apis-boilerplate)
- [ ] [backend] Build the global response interceptor (`{ data, meta, error }` envelope) + RBAC `RolesGuard` (consumes `@Roles()` metadata) + `JwtAuthGuard` that attaches `request.workspaceContext` for `@WorkspaceContext()`, all in `common/` — deferred: cross-cutting auth/common concerns owned by the auth task; Workspace controllers reference them with TODO markers — (raised: 2026-06-09, by: workspace-team-apis-boilerplate)
- [ ] [backend] Implement the WorkOS integration for the Workspace module: `inviteOrganizationMember`, session revocation (deactivation dual-action), and the `organizationMembership.created` webhook → `WorkspaceMemberService.activateInvitedMember` sync — deferred: WorkOS client/identity wiring not built; boilerplate stubs throw NotImplemented — (raised: 2026-06-09, by: workspace-team-apis-boilerplate)
- [ ] [backend] Implement bodies + tests for all WorkspaceService/WorkspaceMemberService methods (provisionWorkspace idempotency, getWorkspace, updateSettings partial-update, inviteMember 403-on-CA_OWNER, updateMember deactivation dual-action) and write AuditLog + AuditEvent via shared AuditService on every mutation — deferred: this task delivered boilerplate only per request (subtasks 86d39mfkd/86d39mfmj/86d39mfp8/86d39mfrd/86d39mftp) — (raised: 2026-06-09, by: workspace-team-apis-boilerplate)
- [ ] [backend] Add `jose` dep + implement the Lambda authorizer (`src/authorizer/`): JWKS fetch/cache (1hr TTL), `verifyToken` (signature + exp/iss/aud), claim→`AuthorizerContext` mapping, IAM Allow/Deny policy — deferred: boilerplate-only; new dep is a Hard Stop trigger; bodies throw (subtask 86d39mg7v) — (raised: 2026-06-09, by: authkit-jwt-rbac-boilerplate)
- [ ] [backend] Implement `GET /v1/workspace/members` list endpoint with cursor pagination (returns all workspace members) — deferred: backend list controller not yet built; frontend hook structure in place — (raised: 2026-06-10, by: team-management-frontend)
- [ ] [frontend] Wire JWT token injection in API hooks (from AuthKit once backend integration complete); currently all calls go unsigned — deferred: backend AuthKit integration pending — (raised: 2026-06-10, by: team-management-frontend)
- [ ] [frontend] Implement workspace provisioning trigger logic (determine when to call POST /v1/workspace on first login) — deferred: auth flow not finalized; endpoint exists on backend — (raised: 2026-06-10, by: team-management-frontend)
- [ ] [frontend] Build workspace settings panel (agent name, escalation threshold, default language) — hooked up via `useUpdateWorkspace()` but no UI surface yet — deferred: feature scope is complete; UI is Phase 2 cleanup — (raised: 2026-06-10, by: team-management-frontend)
- [ ] [infra] Deploy the authorizer Lambda via CDK as a separate function (IAM `logs:*` only) and attach it as the API Gateway REQUEST authorizer on `/v1/*` (+ `$connect`) — extends the nestjs-scaffold "AuthKit JWT Lambda authorizer" item now that the handler boilerplate exists (subtask 86d39mg7v) — (raised: 2026-06-09, by: authkit-jwt-rbac-boilerplate)
- [ ] [backend] Implement the NestJS auth bodies (subtasks 86d39mgac/86d39mgcq/86d39mgf4): `JwtAuthGuard.canActivate`+`mapToAuthContext`, `RolesGuard.canActivate`, `WorkspaceContextInterceptor.intercept` (enter ALS), `applyRlsMiddleware` (`SET LOCAL app.workspace_id`); then **enable the commented-out global registration** in `CommonModule` (APP_GUARD×2 + APP_INTERCEPTOR) and **wire `applyRlsMiddleware(this)`** in `PrismaService.onModuleInit` — deferred: stubs throw, left unregistered so the app boots — (raised: 2026-06-09, by: authkit-jwt-rbac-boilerplate)
- [ ] [backend] Configure the Swagger global BearerAuth security scheme (DOD) — depends on the `@nestjs/swagger` dep already deferred by workspace-team-apis-boilerplate — (raised: 2026-06-09, by: authkit-jwt-rbac-boilerplate)
- [ ] [backend] Auth test suite (DOD): unit — JWT valid/expired/malformed, RolesGuard across all 3 roles + 403, RLS middleware `SET LOCAL` activation; integration — valid token → correct workspace context → RLS isolation end-to-end (workspace_A context returns 0 rows for workspace_B); confirm JWKS cache (no WorkOS call on warm invocations) — deferred: boilerplate only — (raised: 2026-06-09, by: authkit-jwt-rbac-boilerplate)
